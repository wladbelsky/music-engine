...
| `js/dash.js` | `Dash.draw(sim, audio, dt, t)`. Static layer (cable conduits first via `_harness()`: every instrument has a corrugated line, all gather right of the tach and leave through the right edge as one strapped bundle; then bezels, scales, plates, screws, key/pedal mounts) in an offscreen canvas, rebuilt in `_static()` on resize/setting changes. `hitTest(x, y)` → `'key' \| 'pedal' \| null`. |
